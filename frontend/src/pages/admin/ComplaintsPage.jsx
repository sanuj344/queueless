import { adminComplaints } from '../../data/adminMockData';
...
export default function ComplaintsPage() {
  const stats = [
    { label: 'Total Complaints', value: adminComplaints.length, icon: '⚠️' },
    { label: 'Open', value: adminComplaints.filter(c => c.status === 'open').length, icon: '🔓' },
    { label: 'Resolved', value: adminComplaints.filter(c => c.status === 'resolved').length, icon: '✅' },
    { label: 'High Priority', value: adminComplaints.filter(c => c.priority === 'high').length, icon: '🔥' },
  ];
              ...
              <tbody className="divide-y divide-zinc-800/50">
                {adminComplaints.map((complaint) => (
                  <tr key={complaint.id} className="hover:bg-white/5 transition-colors group">
                    <td className="px-6 py-4 text-sm font-mono text-zinc-400">{complaint.id}</td>
                    <td className="px-6 py-4 text-sm font-semibold">{complaint.customer}</td>
                    <td className="px-6 py-4 text-sm text-zinc-400">{complaint.vendor}</td>
                    <td className="px-6 py-4 text-sm text-zinc-300 italic">"{complaint.subject}"</td>
                    ...
                    <td className="px-6 py-4 text-xs text-zinc-500">{complaint.date}</td>
