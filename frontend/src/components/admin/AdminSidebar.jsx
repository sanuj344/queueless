...
export default function AdminSidebar({ onClose }) {
  ...
  return (
    <aside className="w-64 bg-zinc-950 border-r border-zinc-800 flex flex-col h-screen sticky top-0 transition-all duration-300">
      {/* Brand */}
      <div className="p-6 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-1 select-none">
          <span className="text-xl font-black tracking-tight text-white">Queue</span>
          <span className="text-xl font-black tracking-tight text-[#d4ff00]">Less</span>
        </Link>
        <button 
          onClick={onClose}
      ...
      {/* Navigation */}
      <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-800">
        {menuItems.map((item) => {
